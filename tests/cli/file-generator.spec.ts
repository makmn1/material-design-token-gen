...
describe("file-generator", () => {
    ...
    describe("generateRipplesFile", () => {
        ...
        it("includes all expected CSS rules", async () => {
            ...
            expect(content).toContain("animation-duration: 550ms");
            expect(content).toContain("@keyframes sm-ripple-wave");
            expect(content).toContain("60% { transform: scale(1); }");
            expect(content).toContain("100% { transform: scale(1); opacity: 0; }");
